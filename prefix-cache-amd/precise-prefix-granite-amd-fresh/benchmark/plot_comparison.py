#!/usr/bin/env python3
"""Plot TTFT and output tok/s for llm-d vs plain k8s on a freshly-deployed
AMD-only pool (8 MI325X, granite-4.1-8b, llm-d-granite-amd-kv namespace)."""
import json
import pathlib
import matplotlib.pyplot as plt

BASE = pathlib.Path(__file__).parent
LLMD = next(BASE.glob("results-llmd-scaled/**/inference-perf_*_llmd-granite-amd-fresh"))
K8S = next(BASE.glob("results-k8s-scaled/**/inference-perf_*_k8s-granite-amd-fresh"))

# stages: 0=warmup(15), 1..8 = 3,10,15,20,25,30,40,50
# llmd ran an extra stage 9 at rate=60 (k8s saturates there, so k8s was truncated).
STAGE_RATE = {1: 3, 2: 10, 3: 15, 4: 20, 5: 25, 6: 30, 7: 40, 8: 50}
...
plt.tight_layout()
out = BASE / "comparison_amd_fresh.png"
