...
# ibm-granite/granite-4.1-8b on 8 Gaudi3 cards (single node).
#
# Usage:
...
MS_RELEASE="ms-granite-gaudi"
EPP_RELEASE="precise-granite-gaudi"
EPP_SVC="${EPP_RELEASE}-epp"
ZMQ_ALIAS_SVC="gaie-granite-gaudi-epp"
...
deploy_vllm() {
    ...
    cd "$HELMFILE_DIR"
    RELEASE_NAME_POSTFIX=granite-gaudi helmfile -e istio -l "name=$MS_RELEASE" apply -n "$NAMESPACE" --suppress-secrets >/dev/null
}

deploy_infra_and_gaie() {
    log "Installing infra + gaie releases"
    cd "$HELMFILE_DIR"
    RELEASE_NAME_POSTFIX=granite-gaudi helmfile -e istio -l 'name=infra-granite-gaudi' apply -n "$NAMESPACE" --suppress-secrets >/dev/null
    RELEASE_NAME_POSTFIX=granite-gaudi helmfile -e istio -l 'name=gaie-granite-gaudi' apply -n "$NAMESPACE" --suppress-secrets >/dev/null
}
...
cmd_destroy() {
    ...
    (cd "$HELMFILE_DIR" && RELEASE_NAME_POSTFIX=granite-gaudi helmfile -e istio destroy -n "$NAMESPACE" 2>/dev/null) || true
